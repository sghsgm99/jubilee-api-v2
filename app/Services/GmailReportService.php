...
class GmailReportService
{
    ...
    public static function resolve(Account $account): GmailReportService
    {
        $googleReportService = app(self::class);
        $googleReportService->account = $account;

        return $googleReportService;
    }
    ...
    public function getAdsenseReport(): string
    {
        ...
        for ($i=1; $i<count($row); $i++) {
            if (!empty(trim($row[$i]))) {
                ...
                GoogleReportService::create($this->account, (array) $reportData);
            }
        }
